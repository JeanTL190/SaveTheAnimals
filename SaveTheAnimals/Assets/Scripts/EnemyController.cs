...
public class EnemyController : MonoBehaviour
{
    ...
    protected bool CanSeePlayer ()
    {
        ...
        if (distanceToPlayer <= detectionRange)
        {
            ...
            Debug.DrawRay(this.gameObject.transform.position, player.transform.position - this.gameObject.transform.position, Color.magenta);
            if (hit.collider != null)
            {
                distanceToOpaqueObject = Vector2.Distance(this.gameObject.transform.position, hit.point);
                if (distanceToOpaqueObject >= distanceToPlayer)
                {
                    //Debug.Log("Vision not obstructed! Obj: " + distanceToOpaqueObject + " Pla: " + distanceToPlayer + " Name: " + hit.collider.gameObject.name + " Coor: " + hit.collider.gameObject.transform.position.ToString()); ;
                    return true; 
                }
                else
                {
                    //Debug.Log(hit.collider.gameObject.name);
                    //Debug.Log("Vision obstructed! Obj: " + distanceToOpaqueObject + " Pla: " + distanceToPlayer);
                    return false;
                }
            }
            else
